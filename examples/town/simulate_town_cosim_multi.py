...
def micro_callback(ego_ids, init, edges):
    ...
    if init:
        ...
        libsumo.vehicle.add(ego_ids[0], 'ego_route')
        libsumo.vehicle.add(ego_ids[1], 'ego_route')
        libsumo.vehicle.add(ego_ids[2], 'ego_route')
    ...
    veh_count = len(vehs)
    try:
        ego_speed = libsumo.vehicle.getSpeed(ego_ids[0])
        if libsumo.vehicle.couldChangeLane(ego_ids[0], 1) or libsumo.vehicle.couldChangeLane(ego_ids[0], -1):
            changed_lane = 1
        else:
            changed_lane = 0
        headway = libsumo.vehicle.getLeader(ego_ids[0], headway_thd)
    except libsumo.TraCIException:
        ego_speed = 0
        changed_lane = 0
        headway = None
...
def main():
    ...
    parallel_conn.start(micro_cmd, meso_cmd, ego_ids, 50)
    ...
    for i in range(3600):
        ...
        if i == 0:
            parallel_conn.set_callback_arguments((ego_ids, init, edges), micro)
        else:
            parallel_conn.set_callback_arguments((ego_ids, not init, edges), micro)
